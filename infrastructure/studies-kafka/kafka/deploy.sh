...
helm upgrade kafka oci://registry-1.docker.io/bitnamicharts/kafka \
    ...
    --set sasl.client.passwords[1]="$KAFKA_STUDY_SERVICE_BROKER_PASSWORD" \
    --set sasl.client.users[2]="$KAFKA_SCIENTIST_SERVICE_BROKER_USER" \
    --set sasl.client.passwords[2]="$KAFKA_SCIENTIST_SERVICE_BROKER_PASSWORD" \
